...
class Ball():
    ...
    def __init__(self, position_x, position_y, velocity_x, velocity_y, dimensions_x, dimensions_y, game):
    ...
    def draw(self):
        pygame.draw.circle(self.game.window, WHITE, self.position, BALL_RADIUS)
    ...
    def update(self):
        ...
        if self.position_x - self.dimensions_x < -10:
            self.velocity_x *= -1
        if self.position_x + self.dimensions_x > WIDTH + 10:
            self.velocity_x *= -1
